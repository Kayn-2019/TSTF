...
def save_frames_to_video(frames, episode_num, success_rate, resolution=(1000, 1000)):
    ...
    filename = VIDEO_DIR / "different_map" / "3_lane_intersection" / f"tstf_3_lane_intersection_success_{success_rate:.2f}.mp4"
...
def save_frames_to_avi(frames, episode_num, success_rate, resolution=(1000, 1000)):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = VIDEO_DIR / "different_map" / "3_lane_intersection" / f"tstf_3_lane_intersection_success_{success_rate:.2f}.avi"  # 扩展名改为.avi
...
if __name__ == "__main__":
    ...
    # ===== 创建环境 =====
    env_name, env_class = get_tstf_env(MultiAgentIntersectionEnv, return_class=True)
    env_config = dict(
        num_agents=10,
